...
class HBoardService:
    ...
    def run(cls):
        if not HSession.is_login():
            ...
            return
        while True:
            print(f"\n====== MBC 게시판 ({HSession.login_member.name}) ======")
            cls.list_board()
            print("1. 글 쓰기")
    ...
    def write_board(cls):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                cursor.execute(sql, (HSession.login_member.id, title, content))
                conn.commit()
    ...
    def view_detail(cls):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if row['member_id'] == HSession.login_member.id:
